...
visitors_with_nations%>%
group_by(nation)%>%
  count()#这里相当于按照nation分类之后数每一个nation有几个attraction#
```
...
visitors_with_nations%>%
summarise(total_attractions=n(),
          free_attractions=sum(admission=="Free",na.rm = TRUE),
proportion=total_attractions/free_attractions*100
)
#这里的n()相当于返回每一个分组的行数#
...
visitors_with_nations%>%
     ...
     summarise(total_2022=sum(n_2022,na.rm=TRUE),
               total_2021=sum(n_2021,na.rm=TRUE),

proportion=(total_2022-total_2021)/total_2021*100)
