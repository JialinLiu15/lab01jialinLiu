...
visitors %>% 
  count(region)%>%
  #因为每一行都是一个attraction所以count（region）就相当于数以一个region有几行，反映到这道题中就是数每一个region有几个attraction#
  filter(n>50)
...
There might be two potential reasons which cause those errors, one is that the data type is wrong but after checking mean_2021 and med_2022 are both double so the problem might come from some missing variable 
...
visitors %>% 
  summarise(
    ...
    med_202=median(n_2021,na.rm=TRUE)
  )
...
**Write your answer here**
The data set might be sweeked to right so most of the value are small values and a few outliers which are extremly large they raise the mean up.
...
visitors_with_nations%>%
count(nation)#这里相当于按照nation分类之后数每一个nation有几行，一行是一个attraction所以相当于数每个nation有几个attraction#
```
...
group_by(nation)%>%
  count(admission)%>%
  #执行到这一行n就是每一种admission的个数# #这一行直接就返回了每个国家每一种admission的个数#
  mutate(pct=100*n/sum(n))%>%
#这一行sum(n)代表的是每一个国家总的景点数，因为我们在group——by nation 的条件之下所以sum(n)相当于统计国家的总景点数把每个国家每个admission的个数相加，如果我们只执行到mutate sum()那么会返回n后面新增一列对应的是各个国家的总admission。之后pct=是在命名一个算法相当于每一行用这一行的n/sum(n)*100就是对应的百分比#
filter(admission=="Free")%>%
  arrange(desc(pct))
#之后这两行：292行是在选取Free，294行是在降序排列#
...
visitors_with_nations%>%
mutate(pct_change = (n_2022-n_2021)/n_2021)%>% #这一行是利用公式新增一行并计算每一个景点的游客数量百分比变化#
filter(nation=="Scotland")%>%
#之后筛选出所有苏格兰景点#
group_by(admission)%>%
summarise(mean_pct_change=mean(pct_change,na.rm=TRUE))%>%
#这里在group——by——pct的条件之下，对每种admission类型的景点求平均值#
